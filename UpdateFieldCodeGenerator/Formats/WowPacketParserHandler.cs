...
namespace UpdateFieldCodeGenerator.Formats
{
    public class WowPacketParserHandler : UpdateFieldHandlerBase
    {
        ...
        private const string Version = "V9_1_5_40772";
        ...
        public override void BeforeStructures()
        {
            ...
            _source.WriteLine("using WowPacketParser.Proto;");
        ...
        public override void OnStructureBegin(Type structureType, ObjectType objectType, bool create, bool writeUpdateMasks)
        {
            ...
            var protoArgument = IncludeProtobufFields(structureType) ? $" UpdateValues{structureName}Fields fields," : "";
            
            _indent = 2;
            if (_create)
            {
                if (_isRoot)
                    _source.WriteLine($"{GetIndent()}public {methodType} I{structureName} ReadCreate{structureName}(Packet packet, UpdateFieldFlag flags,{protoArgument} params object[] indexes)");
                else
                    _source.WriteLine($"{GetIndent()}public {methodType} I{structureName} ReadCreate{structureName}(Packet packet,{protoArgument} params object[] indexes)");
            }
            else
                _source.WriteLine($"{GetIndent()}public {methodType} I{structureName} ReadUpdate{structureName}(Packet packet,{protoArgument} I{structureName} existingData, params object[] indexes)");
        ...
        public override IReadOnlyList<IStatement> OnField(string name, UpdateField updateField, IReadOnlyList<IStatement> previousControlFlow)
        {
            ...
            var indexLetter = 'i';
            bool includeProtobuf = IncludeProtobufField(_structureType, name);
            if (type.IsArray)
            {
                if (includeProtobuf)
                    flowControl.Add(new SimpleStatement() { Statement = $"fields.{name}.Reserve({updateField.Size});" });
            ...
            _fieldWrites.Add((name, false, (pcf) =>
            {
                ...
                {
                    if (includeProtobuf)
                        _source.WriteLine($"{GetIndent()}fields.{outputFieldName} = data.{outputFieldName}.ToProto();");
                }
                _indent = 3;
                return flowControl;
            }
        ...
        private void WriteField(string name, string outputFieldName, Type type, int bitSize, string nextIndex, Type interfaceType)
        {
            ...
            var protoAssignment = IncludeProtobufField(_structureType, outputFieldName) ? $"fields.{outputFieldName} = " : "";

            switch (Type.GetTypeCode(type))
            {
                case TypeCode.Object:
                    if (type == typeof(WowGuid))
                        _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadPackedGuid128(\"{name}\", indexes{nextIndex});");
                    else if (type == typeof(Bits))
                        _source.WriteLine($"data.{outputFieldName} = packet.ReadBits(\"{name}\", {bitSize}, indexes{nextIndex});");
                    else if (type == typeof(Vector2))
                        _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadVector2(\"{name}\", indexes{nextIndex});");
                    else if (type == typeof(Quaternion))
                        _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadQuaternion(\"{name}\", indexes{nextIndex});");
                ...
                case TypeCode.Boolean:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadBit(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.SByte:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadSByte(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.Byte:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadByte(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.Int16:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadInt16(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.UInt16:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadUInt16(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.Int32:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadInt32(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.UInt32:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadUInt32(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.Int64:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadInt64(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.UInt64:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadUInt64(\"{name}\", indexes{nextIndex});");
                    break;
                case TypeCode.Single:
                    _source.WriteLine($"{protoAssignment}data.{outputFieldName} = packet.ReadSingle(\"{name}\", indexes{nextIndex});");
                    break;
                default:
        ...
        private bool IncludeProtobufFields(Type structureType) => IncludeProtobufField(structureType, null);

        private bool IncludeProtobufField(Type structureType, string? fieldType)
        {
            if (structureType == typeof(CGObjectData))
                return true;

            if (structureType == typeof(CGUnitData))
                return fieldType == null || ProtoUnitFields.Contains(fieldType);
            
            if (structureType == typeof(CGGameObjectData))
                return fieldType == null || ProtoGameObjectFields.Contains(fieldType);
            
            return false;
        }

        private static HashSet<string> ProtoGameObjectFields = new ()
        {
            "CreatedBy", "Flags", "ParentRotation", "FactionTemplate", "State", "TypeID", "PercentHealth", "DisplayID",
            "ArtKit", "Level"
        };

        private static HashSet<string> ProtoUnitFields = new ()
        {
            "DisplayID", "NpcFlags", "SummonedBy", "CreatedBy", "ClassId", "Sex", "Health", "Power", "MaxPower",
            "MaxHealth", "Level", "ContentTuningID", "ScalingLevelMin", "ScalingLevelMax", "ScalingLevelDelta",
            "FactionTemplate", "VirtualItems", "Flags", "Flags2", "Flags3", "AttackRoundBaseTime",
            "RangedAttackRoundBaseTime", "BoundingRadius", "CombatReach", "MountDisplayID", "StandState",
            "PetTalentPoints", "VisFlags", "AnimTier", "CreatedBySpell", "EmoteState", "SheatheState", "PvpFlags",
            "PetFlags", "ShapeshiftForm", "HoverHeight", "InteractSpellID", "StateSpellVisualID", "StateAnimID",
            "StateAnimKitID", "Charm", "Summon", "Critter", "DemonCreator", "LookAtControllerTarget", "Target", "Race",
            "DisplayPower", "EffectiveLevel", "AuraState", "DisplayScale", "CreatureFamily", "CreatureType",
            "NativeDisplayID", "NativeXDisplayScale", "Resistances", "BaseMana", "BaseHealth", "CharmedBy",
        };
    }
}
